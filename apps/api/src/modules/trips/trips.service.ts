...
export class TripsService {
  ...
  async create(dto: CreateTripDto, orgId: string, userId: string) {
    await this.validateVehicleForDispatch(
      dto.vehicleId,
      orgId,
      dto.cargoWeightKg,
    );
  ...
  async dispatch(
    ...
    if (trip.status !== TripStatus.Draft) {
      throw new BadRequestException(
        `Trip is ${trip.status} — only Draft trips can be dispatched`,
      );
    }
      ...
      async (tx) => {
        ...
        if (!vehicle || vehicle.status !== VehicleStatus.Available) {
          throw new ConflictException(
            `Vehicle ${vehicle?.registrationNumber || trip.vehicleId} is no longer available`,
          );
        }
        ...
        if (!driver || driver.status !== DriverStatus.Available) {
          throw new ConflictException(
            `Driver ${driver?.fullName || trip.driverId} is no longer available`,
          );
        }
        ...
        const [updatedTrip] = await Promise.all([
          ...
          tx.vehicleStatusHistory.create({
            data: {
              ...
              newStatus: VehicleStatus.OnTrip,
              changedById: userId,
    ...
    this.gateway.broadcastTripUpdate(orgId, {
  ...
  async complete(
    ...
    if (trip.status !== TripStatus.Dispatched) {
      throw new BadRequestException(
        `Trip is ${trip.status} — only Dispatched trips can be completed`,
      );
    }
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      const [updatedTrip] = await Promise.all([
        ...
        tx.vehicleStatusHistory.create({
          data: {
            vehicleId: trip.vehicleId,
            oldStatus: VehicleStatus.OnTrip,
  ...
  async cancel(id: string, orgId: string, userId: string, reason: string) {
    ...
    if (
      ...
      throw new BadRequestException(
        `Trip is ${trip.status} — cannot be cancelled`,
      );
    }
  ...
  async suggestVehicles(
    ...
    const candidates = await this.prisma.vehicle.findMany({
      ...
      include: {
        ...
        maintenanceLogs: {
          where: {
            createdAt: { gte: new Date(Date.now() - 90 * 24 * 3600000) },
          },
    ...
    const scored = candidates.map((v) => {
      ...
      const totalScore =
        capacityScore + proximityScore + efficiencyScore + maintenanceRisk;
      ...
      return {
        ...
        util,
        avgEfficiency,
        recentMaintenance: v.maintenanceLogs.length,
      };
    });
  ...
  async suggestDrivers(orgId: string, scheduledStart: string) {
    const restThresholdHours = Number(
      this.config.get('DRIVER_REST_THRESHOLD_HOURS') || 2,
    );
    ...
    return candidates
      .map((d) => {
        ...
        return {
          ...
          restWarningMessage,
          licenseExpiryStatus:
            d.licenseExpiryDate < new Date(Date.now() + 30 * 24 * 3600000)
              ? 'expiring_soon'
              : 'valid',
        };
      })
  ...
  private async validateVehicleForDispatch(
    vehicleId: string,
    orgId: string,
    cargoKg: number,
  ) {
    ...
    if (vehicle.status !== VehicleStatus.Available) {
      throw new BadRequestException(
        `Vehicle is ${vehicle.status} — must be Available to dispatch`,
      );
    }
    ...
    if (activeTrip) {
      throw new ConflictException(
        `Vehicle already has an active trip (${activeTrip.tripCode})`,
      );
    }
  }
  ...
  private async validateDriverForDispatch(driverId: string, orgId: string) {
    ...
    if (driver.status !== DriverStatus.Available) {
      throw new BadRequestException(
        `Driver is ${driver.status} — must be Available to dispatch`,
      );
    }
    ...
    if (activeTrip) {
      throw new ConflictException(
        `Driver already has an active trip (${activeTrip.tripCode})`,
      );
    }
  }
    ...
    if (!overrideRestTime) {
      ...
      if (driver?.lastTripCompletedAt) {
        const hoursSinceLast =
          (Date.now() - driver.lastTripCompletedAt.getTime()) / 3600000;
