...
export class TripsController {
  ...
  @Get()
  @Roles(UserRole.Admin, UserRole.FleetManager, UserRole.Dispatcher, UserRole.SafetyOfficer, UserRole.FinancialAnalyst)
  @ApiOperation({ summary: 'List trips with pagination and filtering' })
  async findAll(@Query() query: QueryTripsDto, @CurrentUser('orgId') orgId: string) {
  ...
  @Post(':id/dispatch')
  @Roles(UserRole.Admin, UserRole.Dispatcher)
  @ApiOperation({ summary: 'Dispatch a draft trip (triggers validations and status changes)' })
  async dispatch(
    @Param('id') id: string,
    @Body('overrideRestTime') overrideRestTime: boolean,
    @CurrentUser() user: any,
  ) {
    const data = await this.tripsService.dispatch(id, user.orgId, user.id, overrideRestTime);
  ...
  async complete(@Param('id') id: string, @Body() dto: CompleteTripDto, @CurrentUser() user: any) {
  ...
  async suggestVehicles(
    ...
    const data = await this.tripsService.suggestVehicles(orgId, Number(cargoWeightKg), regionId);
    return { data };
  }

  @Get('suggestions/drivers')
  @Roles(UserRole.Admin, UserRole.Dispatcher)
  @ApiOperation({ summary: 'Get driver recommendations and rest-time warnings' })
